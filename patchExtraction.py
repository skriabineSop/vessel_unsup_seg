    ...
    for i in tqdm(range(0, image.shape[0] - patchShape[0], patchShape[0])):
        for j in range(0, image.shape[1] - patchShape[1], patchShape[1]):
            for k in range(0, image.shape[2] - patchShape[2], patchShape[2]):
                cpt += 1
                np.save(os.path.join(savedir, file[:-8], file[:-8] + '_patch_' + str(i) + '_' + str(j) + '_' + str(k)),
...
for file in os.listdir(workdir):
    if '.tif' in file:
        ...
        if not os.path.exists(os.path.join(savedir, file[:-8])):
            os.makedirs(os.path.join(savedir, file[:-8]))

        from_tif_to_patch(file)